...
```bash
fluent-stitch stitch --glosses HAUS1A WISSEN2B FUSSBALL2 --out stitched.pose --config seg_pad00
# or pass your own .pose files instead of gloss names
fluent-stitch stitch --glosses a.pose b.pose c.pose --out stitched.pose
```
...
├── metrics.py       # DTWp + path-normalized nDTW + length metrics
├── harness.py       # sample sentences → reconstruct → score
├── experiments.py   # named configs, A/B sweep, TSV/JSON logging
