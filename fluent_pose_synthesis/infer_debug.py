...
# -c save/debug_run/1st_train_whole_dataset_output/config.json \
# -r save/debug_run/1st_train_whole_dataset_output/best_model_validation.pt \
# -o save/debug_run/1st_train_whole_dataset_output/validation_infer_output_steps \
# --batch_size 64 \
# --chunk_size 40 \
# --max_len 40 \
# --stop_threshold 1e-4 \
...
def main():
    ...
    parser.add_argument("-i", "--input", required=True, type=str, help="Path to disfluent input data directory (should contain a 'test' split)")
    parser.add_argument("-c", "--config", required=True, type=str, help="Path to model config JSON file (from training)")
    parser.add_argument("-r", "--resume", required=True, type=str, help="Path to model checkpoint (.pt)")
    parser.add_argument("-o", "--output", default="output/infer_results", type=str, help="Directory to save generated outputs")
    ...
    try:
        ...
        with open(args.config, "r", encoding="utf-8") as f:
            config_namespace = eval(f.read(), {"SimpleNamespace": SimpleNamespace, "namespace": SimpleNamespace, "PosixPath": PosixPath, "device": torch.device, "Path": Path}) # type: ignore
    ...
    dataloader = DataLoader(
        ...
        collate_fn=zero_pad_collator, num_workers=0 # num_workers=0 for easier debugging
    )
    print(f"Dataset loaded with {len(dataset)} samples.")

    print(f"[DEBUG infer.py] Raw dataset.input_mean shape: {dataset.input_mean.shape if hasattr(dataset.input_mean, 'shape') else 'N/A'}")
    print(f"[DEBUG infer.py] Raw dataset.input_std shape: {dataset.input_std.shape if hasattr(dataset.input_std, 'shape') else 'N/A'}")
    ...
    model = SignLanguagePoseDiffusion(
        ...
        arch=config.arch.decoder, cond_mask_prob=0,
        device=config.device,
        batch_first=getattr(config.arch, "batch_first", True)
    ).to(config.device)
    ...
    for batch_idx, batch_data in enumerate(dataloader):
        ...
        for i in range(current_actual_batch_size): # Iterate over each logical sample in the batch
            ...
            if "original_disfluent_filepath" in metadata_from_collate and i < len(metadata_from_collate["original_disfluent_filepath"]):
            ...
            elif "original_example_index" in metadata_from_collate and i < len(metadata_from_collate["original_example_index"]):
        ...
        print(f"  Effective disfluent condition shape for model: (B_eff, K, D, T) = ({current_batch_size_for_generation}, {K}, {D}, {T_in})")

        # Start autoregressive generation for this mini-batch
        generated_fluent_seq = torch.empty(current_batch_size_for_generation, K, D, 0, device=device) # (B_eff, K, D, T_gen)
        ...
        for step in range(num_autoregressive_steps):
            ...
            model_kwargs_y = {
                "input_sequence": disfluent_cond_seq_for_generation,
            ...
            with torch.no_grad():
                ...
                for prog_step, sample in enumerate(
                    ...
                    for task_idx, (original_id, _, _) in enumerate(tasks_for_this_batch):
                        ...
                        np.save(
                            save_dir / f"pose_pred_fluent_{filename_base_from_id}_step{prog_step}.npy",
                        ...
                        single_pred = single_pred.reshape(single_pred.shape[0], 1, single_pred.shape[1], single_pred.shape[2])  # (chunk, 1, K, D)
                        # unnormalize
                        ...
                        with open(save_dir / f"pose_pred_fluent_{filename_base_from_id}_step{prog_step}.pose", "wb") as f:
                            pose_obj.write(f)
            ...
            if newly_stopped_sequences.any():
                print(f"    Sequences (effective batch indices) {newly_stopped_sequences.nonzero(as_tuple=True)[0].tolist()} stopped this step.")
        ...
        print(f"  Finished generation for {current_batch_size_for_generation} unique sequences. Final length: {generated_fluent_seq.shape[3]}")

        # --- Save results ---
        pred_fluent_normed_np_bEff_T_K_D = generated_fluent_seq.permute(0, 3, 1, 2).cpu().numpy() # (B_eff, T_final, K, D)
        ...
        for task_idx, (original_id, original_batch_idx, _) in enumerate(tasks_for_this_batch):
            ...
            if T_final_sample > 0 and pose_header is not None:
                ...
                fps_to_use = pose_header.fps if hasattr(pose_header, 'fps') and pose_header.fps > 0 else 25.0
                unnorm_pose_body = NumPyPoseBody(fps=fps_to_use, data=unnorm_pose_data_tpkd, confidence=unnorm_confidence)
            ...
            if T_in_sample > 0 and pose_header is not None:
                ...
                fps_to_use = pose_header.fps if hasattr(pose_header, 'fps') and pose_header.fps > 0 else 25.0
                unnorm_input_body = NumPyPoseBody(fps=fps_to_use, data=unnorm_input_data_tpkd, confidence=input_confidence)
